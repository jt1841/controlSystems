...
float32 theta[500];
float32 reference[500];
int32 thetaTemp;
float32 thetaRef = 0.0;
float32 timeCounter = 0;
...
void parameterCalculation(float32, float32, float32);
...
interrupt void timerISR(void)
{
    ...
    if (timeCounter >= .5 & phase == 1)  //Requirements to transfer to phase 2
    {
        timeCounter = 0;			    //Reset counter
        phase = 2;					    //Change to phase 2
    }
    if (timeCounter >= .5 & phase == 2)  //Requirements to transfer to phase 1
    {
        timeCounter = 0;				//Reset counter
        phase = 1;						//Change to phase 1
    }

    if (timeCounter == 0 & phase == 1)   //Phase 1 parameters
    {
        parameterCalculation(10 * PI, 0, 0);
    }

    if (timeCounter == 0 & phase == 2)   //Phase 2 parameters
    {
        parameterCalculation(0, 10 * PI, 0);
    }

    timeCounter += 0.0002;				//Increment timer based on frequency.


    if (phase == 1)						//Phase 1 State Space Control
    {
        //Setup r(t)
        if (timeCounter <= timeAccel) //Positive Acceleration
            r = thetaInitial + .5*alphaX*(timeCounter - timeInitial)*(timeCounter - timeInitial);
        if ((timeCounter > timeAccel & timeCounter <= timeCruise + timeAccel)) //Cruise Cycle
            r = .5*(thetaInitial + thetaFinal) + omegaX*(timeCounter - .5*(timeInitial + timeFinal));
        if ((timeCounter >= timeCruise + timeAccel & (timeCounter < timeCruise + timeAccel * 2))) //Negative Acceleration
            r = thetaFinal - .5*alphaX*(timeFinal - timeCounter)*(timeFinal - timeCounter);
        if ((timeCounter >= timeCruise + timeAccel * 2 & timeCounter < .5)) //Final Positions
            r = thetaFinal;
    }

    if (phase == 2)						//Phase 2 State Space Control
    {
        if (timeCounter < timeAccel)
            r = thetaInitial - .5*alphaX*(timeCounter - timeInitial)*(timeCounter - timeInitial); //GOOD FOR RESETTING TIME/THETA
        if (timeCounter >= timeAccel & timeCounter < timeAccel + timeCruise)
            r = .5*(thetaInitial + thetaFinal) - omegaX*(timeCounter - .5*(timeInitial + timeFinal)); //GOOD FOR RESTTING TIME/THETA
        if (timeCounter >= timeAccel + timeCruise & timeCounter < timeAccel * 2 + timeCruise)
            r = thetaFinal + .5*alphaX*(timeFinal - timeCounter)*(timeFinal - timeCounter); //GOOD FORASD SDSADASDIQWJRI3HERTHQ2EOFLK
        if (timeCounter >= timeAccel * 2 + timeCruise)
            r = thetaFinal;
    }
    ...
    if (1) //Enable or Disable anti-windup
    {
        if (fabs(u) >= Vmax)
            sigma = sigma;
        else
...
void parameterCalculation(float32 thetaF, float32 thetaI, float32 timeI)
{
    //Calculate Parameters for Phase 1 or Phase 2.
    //This code can also be used for any transition between thetaI and thetaF
    omegaX = 400;
    thetaInitial = thetaI;
    thetaFinal = thetaF;
    alphaX = 5000.0;
    timeInitial = timeI;
    travelTime = (fabs((thetaF - thetaI)) / omegaX + omegaX / alphaX);
    timeAccel = (omegaX / alphaX);
    timeCruise = (fabs((thetaF - thetaI)) / omegaX - omegaX / alphaX);
    timeFinal = (fabs((thetaF - thetaI)) / omegaX + omegaX / alphaX + timeInitial);
}
